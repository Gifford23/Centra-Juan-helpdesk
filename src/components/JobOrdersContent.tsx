...
export default function JobOrdersContent() {
  ...
  const fetchBoardData = useCallback(async () => {
    try {
      ...
      if (data) {
        (data as JobOrderRow[]).forEach((job) => {
          ...
          const assignedTech = job.assigned_tech ?? "";
          const isAssigned = assignedTech && assignedTech !== "Unassigned";
        ...
        if (assignedTechnicians.length > 0) {
          ...
          if (!profileError && profileRows) {
            const avatarMap = (profileRows as PersonnelProfileRow[]).reduce<
              Record<string, string | null>
            >((acc, person) => {
  ...
  }, [fetchBoardData, fetchTechnicians, isSuperAdmin]);
  ...
  const highPriorityJobs = allJobs.filter(
    (job) => job.priority === "High",
  ).length;
  const unassignedJobs = allJobs.filter((job) => !job.techInitials).length;
  const readyJobs = allJobs.filter(
    (job) =>
      job.currentStatus === "Ready" || job.currentStatus === "Ready for Pickup",
  ).length;

  return (
    <div className="relative min-h-[calc(100vh-8rem)] flex flex-col animate-in fade-in duration-500">
      ...
      <div className="relative mb-6 rounded-3xl border border-blue-100/80 bg-gradient-to-br from-white via-blue-50/35 to-cyan-50/25 p-5 sm:p-6 shadow-[0_16px_40px_rgba(30,64,175,0.08)] overflow-visible">
        <div className="absolute right-0 top-0 h-36 w-36 -translate-y-8 translate-x-8 rounded-full bg-blue-200/25 blur-2xl" />
        <div className="absolute left-0 bottom-0 h-28 w-28 -translate-x-8 translate-y-8 rounded-full bg-cyan-100/40 blur-2xl" />
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
            <div className="flex bg-slate-100/80 p-1 rounded-xl border border-slate-200 flex-1 sm:flex-none shadow-sm">
              <button
                ...
                className={`hidden sm:flex flex-1 sm:flex-none px-4 py-2 rounded-lg text-sm font-black items-center justify-center gap-2 transition-all ${viewMode === "board" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                <LayoutGrid className="w-4 h-4" /> Board
              </button>
              <button
                onClick={() => setViewMode("list")}
                className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-sm font-black flex items-center justify-center gap-2 transition-all ${viewMode === "list" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
            ...
            <div className="relative w-full sm:w-auto">
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 px-4 py-2.5 rounded-xl font-black transition-all shadow-sm text-sm active:scale-95"
              >
              ...
              {isFilterOpen && (
                <>
                  ...
                  />
                  <div className="absolute right-0 mt-2 w-full sm:w-52 bg-white border border-slate-200 rounded-xl shadow-xl z-20 py-1">
                    {statusOptions.map((option) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm font-medium transition-colors ${statusFilter === option ? "bg-blue-50 text-blue-700" : "text-slate-700 hover:bg-slate-50"}`}
                      >
                        {option}
                      </button>
                    ))}
      ...
      {isLoading ? (
        <div className="flex-1 flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white/80">
          <Loader2 className="w-10 h-10 text-blue-600 animate-spin mb-4" />
          <p className="text-slate-500 font-semibold">Loading job orders...</p>
        </div>
      ) : (
        <>
          {/* ==========================================
          ...
          {viewMode === "board" && (
            <div className="hidden md:block flex-1 overflow-x-auto pb-2 sm:pb-4 -mx-1 sm:mx-0">
              <div className="flex gap-4 sm:gap-6 h-full min-w-[1080px] px-1 sm:px-0">
                {/* COLUMN 1: RECEIVED */}
                <div className="flex-1 flex flex-col bg-white/90 rounded-2xl border border-slate-200/80 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
                  <div className="flex items-center justify-between mb-4 px-1">
                    <div className="flex items-center gap-2">
                      <div className="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-sm shadow-blue-500/50"></div>
                      <h3 className="font-black text-slate-900">Received</h3>
                      <span className="bg-slate-100 text-slate-600 text-xs font-black px-2 py-0.5 rounded-full">
                  ...
                  <div className="flex-1 overflow-y-auto space-y-3 pr-1 pb-20">
                    {filteredBoardData.received.map((job) => (
                      <KanbanCard
                        ...
                        technicianAvatars={technicianAvatars}
                ...
                <div className="flex-1 flex flex-col bg-white/90 rounded-2xl border border-slate-200/80 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
                  <div className="flex items-center justify-between mb-4 px-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-black text-slate-900">Diagnosing</h3>
                      <span className="bg-slate-100 text-slate-600 text-xs font-black px-2 py-0.5 rounded-full">
                ...
                <div className="flex-1 flex flex-col bg-white/90 rounded-2xl border border-slate-200/80 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
                  <div className="flex items-center justify-between mb-4 px-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-black text-slate-900">In Repair</h3>
                      <span className="bg-slate-100 text-slate-600 text-xs font-black px-2 py-0.5 rounded-full">
                ...
                <div className="flex-1 flex flex-col bg-white/90 rounded-2xl border border-slate-200/80 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
                  <div className="flex items-center justify-between mb-4 px-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-black text-slate-900">Ready</h3>
                      <span className="bg-slate-100 text-slate-600 text-xs font-black px-2 py-0.5 rounded-full">
          ...
          ========================================== */}
          {viewMode === "list" && (
            <div className="bg-white rounded-3xl border border-slate-200/80 shadow-[0_12px_28px_rgba(15,23,42,0.06)] overflow-hidden flex-1 flex flex-col mb-4">
                  ...
                  >
                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      <div className="pt-1">
                        {job.techInitials ? (
                          <div className="flex items-center gap-2">
                            {technicianAvatars[job.techFullName] ? (
                              <img
                                src={technicianAvatars[job.techFullName] || ""}
                                alt={job.techFullName}
              ...
              <div className="hidden md:block overflow-x-auto flex-1">
                <table className="w-full min-w-[880px] text-left border-collapse whitespace-nowrap">
                  <thead>
                    <tr className="text-xs uppercase tracking-wider border-b border-slate-200">
                      <th className="sticky top-0 z-10 px-4 sm:px-7 py-4 font-black text-[11px] tracking-[0.13em] text-slate-700 bg-slate-100 border-b border-slate-200">
                        Job Order
                      </th>
                      <th className="sticky top-0 z-10 px-4 sm:px-7 py-4 font-black text-[11px] tracking-[0.13em] text-slate-700 bg-slate-100 border-b border-slate-200">
                        Priority
                      ...
                      <th className="sticky top-0 z-10 px-4 sm:px-7 py-4 font-black text-[11px] tracking-[0.13em] text-slate-700 bg-slate-100 border-b border-slate-200 text-right">
                        Actions
                      </th>
                    </tr>
                  </thead>
...
function KanbanCard({
  ...
  job: JobCardData;
  technicians: string[];
  technicianAvatars: Record<string, string | null>;
  onUpdate: () => void;
  ...
  return (
    ...
    >
      ...
      <h4 className="font-black text-slate-900 mb-1 text-sm leading-tight">
        {job.device}
      </h4>
      ...
      <div className="flex justify-between items-center pt-3 border-t border-slate-100">
        <div>
          <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold">
            ...
            {isRecentlySubmitted(job.createdAt) && (
              <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-200">
                New
              </span>
            )}
          </div>
          ...
          <>
            {technicianAvatars[job.techFullName] ? (
              <img
                src={technicianAvatars[job.techFullName] || ""}
                alt={job.techFullName}
                className="w-6 h-6 rounded-full object-cover shadow-sm ring-2 ring-white"
...
function JobTableRow({
  ...
  return (
    <tr className="hover:bg-blue-50/35 transition-colors group relative">
      <td className="px-4 sm:px-7 py-5 font-black text-slate-900">#{job.id}</td>
      ...
      <td className="px-4 sm:px-7 py-5">
        <p className="font-black text-sm text-slate-900">{job.device}</p>
      ...
      <td className="px-4 sm:px-7 py-5 text-sm font-medium text-slate-500">
        <div className="flex items-center gap-2">
          <span>{job.time}</span>
          {isRecentlySubmitted(job.createdAt) && (
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-200">
...
function ActionMenu({
  ...
  return (
    ...
    >
      ...
      <button
        onClick={toggleMenu}
        className={`p-1.5 rounded-lg transition-colors ${isMenuOpen ? "bg-blue-50 text-blue-600" : "text-slate-400 hover:text-slate-900 hover:bg-slate-100"}`}
      >
        <MoreHorizontal className="w-5 h-5" />
      </button>
      ...
      {isMenuOpen && (
        <>
          ...
          ></div>
          <div className="absolute right-0 top-8 mt-1 w-52 bg-white border border-slate-200 rounded-xl shadow-xl shadow-slate-200/60 z-30 py-1 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {menuView === "main" && (
              <>
                <p className="px-3 py-2 text-xs font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 mb-1">
                  Move to...
                </p>
                <button
                  onClick={() => updateField("status", "Received")}
                  className="w-full text-left px-4 py-2 text-sm font-medium text-slate-700 hover:bg-blue-50 hover:text-blue-700 flex items-center gap-2"
                >
                  <Package className="w-4 h-4 text-slate-400" /> Received
                </button>
                <button
                  onClick={() => updateField("status", "Diagnosing")}
                  className="w-full text-left px-4 py-2 text-sm font-medium text-slate-700 hover:bg-purple-50 hover:text-purple-700 flex items-center gap-2"
                >
                  <AlertCircle className="w-4 h-4 text-purple-400" /> Diagnosing
                </button>
                <button
                  onClick={() => updateField("status", "In Progress")}
                  className="w-full text-left px-4 py-2 text-sm font-medium text-slate-700 hover:bg-amber-50 hover:text-amber-700 flex items-center gap-2"
                >
                  <Wrench className="w-4 h-4 text-amber-400" /> In Repair
                </button>
                <button
                  onClick={() => updateField("status", "Ready")}
                  className={`w-full text-left px-4 py-2 text-sm font-medium text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 flex items-center gap-2 mb-1 ${isSuperAdmin ? "border-b border-slate-100 pb-3" : "pb-2"}`}
                >
                    ...
                    <>
                      ...
                      {technicians.map((tech, index) => (
                        <button
                          ...
                          className="w-full text-left px-4 py-2 text-sm font-medium text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 flex items-center gap-2"
                        >
                          {technicianAvatars[tech] ? (
                            <img
                              src={technicianAvatars[tech] || ""}
                              alt={tech}
